...
mkinitcpio -p linux
grub-install
sed -i -e 's/^GRUB_CMDLINE_LINUX=""/GRUB_CMDLINE_LINUX="${GRUB_PARAMS_CRYPTO}"/' -e 's/^GRUB_CMDLINE_LINUX_DEFAULT=""/GRUB_CMDLINE_LINUX_DEFAULT="${GRUB_PARAMS}"/' -e '' /etc/default/grub
